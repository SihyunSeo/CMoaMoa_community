	...
	<div class="body-box">
		      ...
		      String req_no = request.getParameter("board_no");
				...
				<% } else if ((req_no.equals("17")) || (req_no.equals("18")) || (req_no.equals("19")) || (req_no.equals("20")) || (req_no.equals("21")) || (req_no.equals("22")) || (req_no.equals("23"))) {%>
						...
						<div class="board_btn_box">
							...
							<button onclick="location.href='/CMoaMoa/freeBoard/freeboard.jsp?board_no=23&&maxPage=1&&nowPage=1s'">보건행정헬스케어학부</button>
						</div>
						<script>					
					...
					<select class="form-control" id="orderby"  onchange="if(this.value) location.href='/CMoaMoa/order_board.jsp?orderby='+(this.value)+'&board_no='+<%=req_board_no%>+'&&maxPage=1&&nowPage=1';">
						<%if (get_type.equals("asc_views")){
							%>
			                  ...
			                  <option value="asc_views" selected="selected">조회수 오름차순</option>
			                  ...
			                  <option value="desc_views" selected="selected">조회수 내림차순</option>
			                  <%
			                  }else if(get_type.equals("asc_date")) {
			                     %>
			                  <option value="asc_date" selected="selected">날짜 오름차순</option>
			                  <option value="desc_date">날짜 내림차순</option>
			                  <option value="asc_views">조회수 오름차순</option>
			                  <option value="desc_views">조회수 내림차순</option>
			                  <%
			                     ...
			                     %>
			                  <option value="asc_date" >날짜 오름차순</option>
			                  <option value="desc_date" selected="selected">날짜 내림차순</option>
			                  <option value="asc_views">조회수 오름차순</option>
			                  <option value="desc_views">조회수 내림차순</option>
						<%
						}  %>
		...
		// 			for(int i = maxPage; i > 0; i --) {
			...
			<%
				...
				<%} else {%>
					<div>
					  	<button class="write-btn" type=button onclick="location.href='/CMoaMoa/write_board.jsp?board_no=0'">글쓰기</button>
					</div>
			<%} %>
